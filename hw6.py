...
def remove_range(alist, min, max):
    ...
    *** Important: your code must use comprehensions and should not be more than
    ...
    """
    answ = [a for a in alist if a < min or a > max] 
    return answ
...
def friend_list(friend_dictionary):
    ...
    def friendlist(dict):
        ...
        for name, friend in friend_dictionary.items():
            if name not in answ:
                answ[name] = []
            if friend not in answ:
                answ[friend] = []
            answ[name].append(friend)
            answ[friend].append(name)
        return answ

    return friendlist(friend_dictionary)
